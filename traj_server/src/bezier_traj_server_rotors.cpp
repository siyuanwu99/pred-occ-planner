...
ros::Publisher pos_odom_pub_;

constexpr double DELTA_T     = 0.01;  // 100 Hz
constexpr double YAW_DOT_MAX = 0.05;  // max yaw rate
...
void getYaw(const Eigen::Vector3d &v, double &yaw, double &yaw_dot) {
  ...
  constexpr double MAX_YAW_CHANGE = YAW_DOT_MAX * 0.01;
...
void publishOdom(const Eigen::Vector3d &pos,
  ...
  pos_odom_pub_.publish(odom_msg);
}
...
void PubCallback(const ros::TimerEvent &e) {
  ...
  if (!is_yaw_initilized_) { /* if yaw is not initialized, turn to desired yaw */
    ...
    if (last_yaw_ < 0 && last_yaw_ > -3.12) {
      p.yaw     = current_yaw - YAW_DOT_MAX;
      p.yaw_dot = -YAW_DOT_MAX;
  ...
  ROS_INFO("[dbg] t: %.2f | pos: %.2f %.2f %.2f | vel: %.2f %.2f %.2f", p.t.toSec() - ti_,
  ...
  publishOdom(p.pos, p.vel, p.acc, p.yaw, p.yaw_dot);
...
int main(int argc, char **argv) {
  ...
  pos_odom_pub_ = nh.advertise<nav_msgs::Odometry>("position_control", 1);
