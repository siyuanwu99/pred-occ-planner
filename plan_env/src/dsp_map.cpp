...
int DSPMapStatic::update(int    point_cloud_num,
  ...
  if (fabs(sensor_quaternion_w) > 1.001f || fabs(sensor_quaternion_x) > 1.001f ||
      fabs(sensor_quaternion_y) > 1.001f || fabs(sensor_quaternion_z) > 1.001f) {
  ...
  if (fabs(odom_delt_px) > 10.f || fabs(odom_delt_py) > 10.f || fabs(odom_delt_pz) > 10.f ||
      delt_t < 0.f || delt_t > 10.f) {
    cout << "!!! delt_t = " << delt_t << endl;
    cout << "!!! sensor_px_last = " << sensor_px_last << "sensor_px = " << sensor_px
         << " odom_delt_px=" << odom_delt_px << endl;
    cout << "!!! sensor_py_last = " << sensor_py_last << "sensor_py = " << sensor_py
         << " odom_delt_py=" << odom_delt_py << endl;
    return 0;
  }
  ...
  for (int p_seq = 0; p_seq < point_cloud_num; ++p_seq) {
    ...
    rotateVectorByQuaternion(&point_cloud_ptr[iter_num], sensor_rotation_quaternion,
                             rotated_point_this);
...
void DSPMapStatic::setPredictionVariance(float p_stddev, float v_stddev) {
...
void DSPMapStatic::setObservationStdDev(float ob_stddev) {
...
void DSPMapStatic::setLocalizationStdDev(float lo_stddev) {
...
void DSPMapStatic::setParticleRecordFlag(int record_particle_flag, float record_csv_time) {
  if_record_particle_csv = record_particle_flag;
  record_time            = record_csv_time;
}

void DSPMapStatic::getOccupancyMap(int &                           obstacles_num,
...
void DSPMapStatic::getOccupancyMapWithVelocity(int &                              obstacles_num,
...
void DSPMapStatic::getOccupancyMapWithFutureStatus(int &                           obstacles_num,
                                                   pcl::PointCloud<pcl::PointXYZ> &cloud,
                                                   float *                         future_status,
                                                   const float                     threshold) {
  obstacles_num = 0;
...
void DSPMapStatic::getOccupancyMapWithRiskMaps(int &                           obstacles_num,
                                               pcl::PointCloud<pcl::PointXYZ> &cloud,
                                               float *                         risk_maps,
                                               const float                     threshold) {
  obstacles_num = 0;
...
void DSPMapStatic::clearOccupancyMapPrediction() {
  for (int i = 0; i < voxels_total_num; i++) {
    for (int j = 4; j < voxels_objects_number_dimension; ++j) {
      voxels_objects_number[i][j] = 0.f;
    }
...
void DSPMapStatic::getKMClusterResult(pcl::PointCloud<pcl::PointXYZINormal> &cluster_cloud) {
  for (auto &point : *input_cloud_with_velocity) {
    cluster_cloud.push_back(point);
  }
}

void DSPMapStatic::setInitParameters() {
  /*** Set parameters **/
  sigma_update = sigma_observation;
...
void DSPMapStatic::addRandomParticles(int particle_num, float avg_weight) {
...
void DSPMapStatic::mapPrediction(float odom_delt_px,
                                 float odom_delt_py,
                                 float odom_delt_pz,
                                 float delt_t) {
  ...
  for (int v_index = 0; v_index < VOXEL_NUM; ++v_index) {
    for (int p = 0; p < SAFE_PARTICLE_NUM_VOXEL; p++) {
      if (voxels_with_particle[v_index][p][0] > 0.1f &&
          voxels_with_particle[v_index][p][0] < 6.f) {  /// exsit, but not new moved
        voxels_with_particle[v_index][p][0] = 1.f;      // If valid, remove resample flag.
        ++operation_counter;

        if (fabs(voxels_with_particle[v_index][p][1] * voxels_with_particle[v_index][p][2] *
  ...
  if (moves_out_counter > 10000) {
    ...
    cout << "odom_delt_px = " << odom_delt_px << " odom_delt_py = " << odom_delt_py
         << "odom_delt_pz=" << odom_delt_pz << endl;
  }
}

void DSPMapStatic::mapUpdate() {
    ...
    for (int j = 0; j < observation_num_each_pyramid[i]; ++j) {
      ...
      for (int n_seq = 0; n_seq < observation_pyramid_neighbors[i][0]; ++n_seq) {
        int pyramid_check_index =
            observation_pyramid_neighbors[i][n_seq + 1];  // 0.neighbors num 1-9:neighbor indexes
    ...
    for (int inner_seq = 0; inner_seq < SAFE_PARTICLE_NUM_PYRAMID; inner_seq++) {
      ...
      if (pyramids_in_fov[current_pyramid_index][inner_seq][0] &
        ...
        for (int neighbor_seq = 0; neighbor_seq < neighbor_num; ++neighbor_seq) {
          int neighbor_index =
              observation_pyramid_neighbors[current_pyramid_index][neighbor_seq + 1];
...
void DSPMapStatic::mapAddNewBornParticlesByObservation() {
    ...
    if (getParticleVoxelsIndex(p_corrected.x, p_corrected.y, p_corrected.z, point_voxel_index)) {
      ...
      for (int kk = 0; kk < SAFE_PARTICLE_NUM_VOXEL; ++kk) {
        if (voxels_with_particle[point_voxel_index][kk][0] > 0.9f &&
            voxels_with_particle[point_voxel_index][kk][0] < 14.f) {  // not new born
...
void DSPMapStatic::mapOccupancyCalculationAndResample() {
        ...
        if (voxels_with_particle[v_index][p][7] < 1e-3) {
          ...
          if (voxels_with_particle[v_index][p][0] < 10.f) {  // exclude new-born particles
            ...
            for (int times = 0; times < PREDICTION_TIMES; ++times) {
              ...
              px_future             = voxels_with_particle[v_index][p][4] +
                          voxels_with_particle[v_index][p][1] * prediction_time;
              py_future = voxels_with_particle[v_index][p][5] +
                          voxels_with_particle[v_index][p][2] * prediction_time;
              pz_future = voxels_with_particle[v_index][p][6] +
...
int DSPMapStatic::getParticleVoxelsIndex(const Particle &p, int &index) {
...
int DSPMapStatic::getParticleVoxelsIndex(const float &px, const float &py, const float &pz, int &index) {
...
void DSPMapStatic::getVoxelPositionFromIndex(const int &index, float &px, float &py, float &pz) const {
...
int DSPMapStatic::ifParticleIsOut(const Particle &p) const {
  if (p.px >= map_length_x_half || p.px <= -map_length_x_half || p.py >= map_length_y_half ||
      p.py <= -map_length_y_half || p.pz >= map_length_z_half || p.pz <= -map_length_z_half) {
    return 1;
  } else
    return 0;
}

int DSPMapStatic::ifParticleIsOut(const float &px, const float &py, const float &pz) const {
  if (px >= map_length_x_half || px <= -map_length_x_half || py >= map_length_y_half ||
      py <= -map_length_y_half || pz >= map_length_z_half || pz <= -map_length_z_half) {
    return 1;
  } else
    return 0;
}

void DSPMapStatic::generateGaussianRandomsVectorZeroCenter() const {
...
float DSPMapStatic::getPositionGaussianZeroCenter() {
...
float DSPMapStatic::getLocalizationGaussianZeroCenter() {
...
float DSPMapStatic::getVelocityGaussianZeroCenter() {
...
int DSPMapStatic::addAParticle(const Particle &p, const int &voxel_index) const {
  for (int i = 0; i < SAFE_PARTICLE_NUM_VOXEL; i++) {
...
int DSPMapStatic::moveParticle(const int &new_voxel_index,
  ...
  // full.
  if (ifInPyramidsArea(voxels_with_particle[new_voxel_index][new_voxel_inner_index][4],
    ...
    if (fabs(voxels_with_particle[new_voxel_index][new_voxel_inner_index][1] *
             voxels_with_particle[new_voxel_index][new_voxel_inner_index][2] *
             voxels_with_particle[new_voxel_index][new_voxel_inner_index][3]) < 1e-6) {
      // keep small, for static obstacles
      //                cout << "keeped"<<endl;
    } else {
...
int DSPMapStatic::ifInPyramidsArea(float &x, float &y, float &z) {
...
int DSPMapStatic::findPointPyramidHorizontalIndex(
...
int DSPMapStatic::findPointPyramidVerticalIndex(
    float &x, float &y, float &z) {  /// The point should already be inside of Pyramids Area
  float last_dot_multiply =
...
void DSPMapStatic::velocityEstimationThread() {
...
void DSPMapStatic::getVoxelPositionFromIndexPublic(const int &index, float &px, float &py, float &pz) const {
  static const int z_change_storage_taken = voxel_num_y * voxel_num_x;
  static const int y_change_storage_taken = voxel_num_x;
...
int DSPMapStatic::getPointVoxelsIndexPublic(const float &px, const float &py, const float &pz, int &index) {
  if (ifParticleIsOut(px, py, pz)) {
    return 0;
  }
  auto x = (int)((px + map_length_x_half) / voxel_resolution);
